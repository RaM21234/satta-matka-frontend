...
const Entry = () => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/user/entry/${id}`, {
                method: 'DELETE',
                headers: {
                    'auth-token': `${localStorage.getItem('auth-token')}`,
                },
        ...
        <>
            ...
            <div className="flex items-center justify-center mt-16">
                <div className="w-full max-w-md mx-auto bg-white p-8 shadow-xl ring-1 ring-gray-900/5 sm:rounded-xl">
                    <h1 className="text-3xl font-semibold text-center text-gray-900 mb-4">Entry Form</h1>
                    <form onSubmit={formik.handleSubmit}>
                        <div className="mb-4">
                            <label htmlFor="entryText" className="block text-sm font-medium text-gray-700">Entry</label>
                            <input
                                type="text"
                                name="entryText"
                        ...
                        {formik.touched.entryText && formik.errors.entryText ? (
                            <div className="text-red-600 text-sm mt-1">{formik.errors.entryText}</div>
                        ) : null}
                        <div className="flex justify-center mt-6">
                            <button type="submit" className="w-full rounded-md bg-black px-4 py-2 text-white focus:bg-gray-600 focus:outline-none disabled:opacity-50" disabled={formik.isSubmitting}>
                                ...
                                {editingEntryId === entry._id ? (
                                    ...
                                    <form onSubmit={(e) => {
                                        ...
                                        <input
                                            ...
                                            required
                                        />
                                        <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 ml-2 rounded focus:outline-none focus:ring focus:ring-blue-300 mt-3">Save</button>
                                    </form>
                                ) : (
